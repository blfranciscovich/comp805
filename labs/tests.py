...
def test_sum_range_five_to_hundred_by_five(results_dict):
    """
    Checks that lab1.sum_range_five_to_hundred_by_five() returns a the correct value
    """
    print("")
    print("Testing lab1.sum_range_five_to_hundred_by_five()")
    message = "----"
    try:
        result = lab1.sum_range_five_to_hundred_by_five()
        if not isinstance(result, int):
            message += 'Did not return an integer - Fail'
            test_failed(results_dict)
        else:
            message += 'An integer was returned - Pass'
            test_passed(results_dict)
            # now check if the value is correct, should be 1050
            if result != 1050:
                message += '\n----Did not return the correct number - Fail'
                test_failed(results_dict)
            else:
                message += '\n----The correct value (1050) was returned - Pass'
                test_passed(results_dict)
    except AttributeError:
        message += 'Could not find lab1.sum_range_five_to_hundred_by_five function - Fail'
        test_failed(results_dict)
    print(message)

def test_count_letters(results_dict):
    """
    Check that lab1.count_letters() returns the lenght of word.
    """
    print("")
    print("Testing lab1.count_letters()")
    message = "----"
    try:
        result = lab1.count_letters('bridget')
        if not isinstance(result, int):
            message += 'Did not return an integer - Fail'
            test_failed(results_dict)
        else:
            message += 'An integer was returned - Pass'
            test_passed(results_dict)
            # now check if the value is correct, should be 7
            if result is not 7:
                message += '\n----Did not return the correct number - Fail'
                test_failed(results_dict)
            else:
                message += '\n----The correct value (7) was returned - Pass'
                test_passed(results_dict)
    except AttributeError:
        message += 'Could not find lab1.count_letters function - Fail'
        test_failed(results_dict)
    print(message)
...
def run():
    ...
    test_sum_range_five_to_hundred_by_five(results_dict)
    test_count_letters(results_dict)
